...
class Calculadora(tk.Tk):
    def __init__(self):
        ...
        self.memoria = None  # Variable de memoria
    ...
    def crear_widgets(self):
        ...
        botones = [
            ...
            ('MR', 7, 0), ('M+', 7, 1), ('MC', 7, 2)  # Botones de memoria
        ]
    ...
    def crear_boton(self, texto, row, col):
        boton = ttk.Button(self, text=texto, command=lambda: self.on_boton_click(texto))
    ...
    def on_boton_click(self, texto):
        if texto == '=':
            ...
            self.memoria = float(self.resultado_var.get())  # Memoria Sumar
        elif texto == 'MR':
            if self.memoria is not None:
                self.resultado_var.set(self.memoria)  # Memoria Recuperar
        elif texto == 'MC':
            self.memoria = None  # Memoria Limpiar
        elif texto == '!':
            ...
            self.resultado_var.set(resultado)
            registrar_operacion(f"sin({num})", resultado)
            guardar_historial() 
        elif texto == 'cos':
            num = float(self.resultado_var.get())
            resultado = math.cos(math.radians(num))
            self.resultado_var.set(resultado)
            registrar_operacion(f"cos({num})", resultado)
            guardar_historial()
        elif texto == 'tan':
            num = float(self.resultado_var.get())
            resultado = math.tan(math.radians(num))
            self.resultado_var.set(resultado)
            registrar_operacion(f"tan({num})", resultado)
            guardar_historial()
        elif texto == '%':
            num = float(self.resultado_var.get())
            resultado = num / 100
            self.resultado_var.set(resultado)
            registrar_operacion(f"{num}%", resultado)
            guardar_historial() 
        else:
